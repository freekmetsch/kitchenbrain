	...
	<div transition:slide={{ duration: 180 }} class="mt-2.5 rounded-xl border border-base-300/70 bg-base-200/40 p-3">
		<div class="grid grid-cols-2 gap-2.5">
			...
			<label class="flex flex-col gap-1">
				...
				<input class="input input-bordered input-sm w-full" placeholder="g · ml · stuk · portion" bind:value={draft.unit} />
			</label>
			<label class="flex flex-col gap-1">
			...
			<label class="flex flex-col gap-1">
				<span class="ui-field-label">Best before</span>
		...
		<div class="mt-3 flex items-center justify-between">
			<button type="button" class="btn btn-ghost btn-sm h-8 min-h-0 px-2.5 text-error" onclick={() => onDelete()}>Remove</button>
		...
		{#if matches.length > 0}
			<div class="mt-3 border-t border-base-300/70 pt-3">
				<p class="mb-1.5 ui-section-label">
					Used in {matches.length} {matches.length === 1 ? 'recipe' : 'recipes'}
				</p>
				<ul class="space-y-1">
		...
		{#if history?.length}
			<div class="mt-3 border-t border-base-300/70 pt-3">
				<p class="mb-1.5 ui-section-label">History</p>
