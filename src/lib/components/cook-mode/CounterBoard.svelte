<script lang="ts">
	...
	type Props = {
		...
		paletteByIngredient?: Record<string, BeatPalette>;
...
<section class="rounded-2xl border border-base-300/70 bg-base-100 p-3 shadow-sm" aria-labelledby="counter-heading">
	...
	<ul class="flex flex-wrap gap-1.5 md:grid md:grid-cols-1">
		{#each ingredients as ingredient}
			...
			{@const palette = paletteByIngredient[ingredientId]}
			<li class="min-w-0">
				<div class="flex min-h-11 items-center rounded-full border pl-1 pr-1.5 {palette?.soft ?? 'bg-base-100'} {palette?.border ?? 'border-base-300'} md:rounded-xl">
					<button
						type="button"
					...
					>
						...
						<span class="min-w-0 text-sm leading-tight {checks[ingredientId] ? 'text-base-content/55' : ''}">
							...
							{#if streamLabelsByIngredient[ingredientId]}
								<span class="mt-0.5 block text-[10px] {palette?.text ?? 'text-base-content/45'}">{streamLabelsByIngredient[ingredientId]}</span>
							{/if}
						</span>
					</button>
					{#if ingredient.substitutes?.length && canonical?.substitutes?.length}
						<button
							type="button"
							class="btn btn-ghost btn-xs h-11 min-h-0 rounded-full bg-base-100/60 px-2 text-[11px] {palette?.text ?? ''}"
