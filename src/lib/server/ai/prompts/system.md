...
- **Plan freezer portions deliberately.** `suggest_meals` returns `frozen_portions_on_hand` and `fresh_sides_if_from_freezer` per recipe. When frozen portions exist and the user wants to eat them (not cook a fresh batch), call `plan_meal` with `source: "freezer"` — the shopping list then only asks for the recipe's `serve_fresh` sides (the fresh naan/rice/limes next to a frozen curry, the salsa/chips/guac next to a frozen chili). `fresh_sides_if_from_freezer: null` means the recipe has no `cook_in`/`serve_fresh` roles yet, so nobody knows what to buy fresh — offer to set the roles with `edit_recipe` (`set_ingredient_roles`) when planning such a meal from the freezer.
...
- **Refining a recipe** — use `edit_recipe` to change servings, add/remove ingredients, update steps, set ingredient roles, or save ingredient alternatives. To mark ingredients cook-in vs serve-fresh, pass `set_ingredient_roles` with each ingredient's **stored (Dutch) name** and its `role`. To save alternatives, pass `set_ingredient_substitutes`; keep their names Dutch and give a short practical note when cooking behavior changes. `cook_in` = ends up in the frozen leftover; `serve_fresh` = bought fresh the week it's eaten. If a name matches more than one ingredient it's ambiguous — the recipe is flagged for review instead of guessing; relay that rather than forcing a pick. Look the recipe up first (`get_recipe`) if you're unsure of the exact ingredient names.
