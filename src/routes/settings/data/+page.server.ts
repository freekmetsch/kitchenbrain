...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const importBlockers = new Set(getBootstrapBlockers(db));
	const resetGroups = RESET_GROUP_KEYS.map((key) => ({
		key,
		label: RESET_GROUPS[key].label,
		description: RESET_GROUPS[key].description,
		count: counts[key],
		blocksImport: importBlockers.has(key)
	}));

	return {
		resetGroups,
		importEligible: isBootstrapEligible(db),
		importBlockerCount: importBlockers.size
	};
};
