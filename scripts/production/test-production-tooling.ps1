...
Assert-True ($stager -match "'RemoveRecipeTimerAlerts'") 'Railway stager lacks the fixed timer-alert removal profile'
Assert-True ($stager -match "\[ValidateSet\('VAPID_PUBLIC_KEY', 'VAPID_PRIVATE_KEY', 'VAPID_SUBJECT'\)\]") 'Railway timer-alert deletion is not structurally allowlisted'
Assert-True ($stager.Contains("'variable', 'delete', `$VariableName")) 'Railway stager lacks the output-suppressed deletion route'
Assert-True ($stager -notmatch "'variable', 'list'" -and $stager -notmatch "'--kv'" -and $stager -notmatch "'--json'") 'Railway stager can list or serialize production variable values'
