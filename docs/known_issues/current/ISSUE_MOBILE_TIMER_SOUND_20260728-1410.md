...
| 2026-07-28 | Traced notification construction and ran the four focused timer-alert suites. | All 11 tests passed, but the notification test explicitly requires `silent: true` whenever any same-origin window is visible. That deterministically silences the Test alert and a timer fired after navigating from Cook Mode to another visible Keukenbrein page. The worker has no proof that the page which caused suppression will play foreground audio. | Replace origin-wide visibility inference with a positive acknowledgement from the matching live Cook Mode timer; make test notifications audible and lock this behavior at the service-worker seam. |
